...
install_dependencies() {
  ...
  apt install -y python3 python3-pip python3-venv python3-dev \
    build-essential libmysqlclient-dev git curl make || error "Failed to install dependencies"
}
...
compile_grpc() {
  ...
  make build-setup || error "Failed to compile gRPC"
}
...
install_systemd_service() {
  ...
  for service in relaysms-publisher.target relaysms-publisher-rest.service relaysms-publisher-grpc.service; do
